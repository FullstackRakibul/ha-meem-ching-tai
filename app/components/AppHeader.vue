<template>
  <!-- <header class="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-2rem)] transition-all duration-500 ease-in-out
    ...
    <div class="flex items-center justify-between w-full gap-2">
      ...
      <nav
        class="flex items-center justify-center gap-6 lg:gap-8 font-medium tracking-wide uppercase text-primary transition-all duration-500 ease-in-out flex-1"
        :class="scrolled ? 'text-xs' : 'text-sm'">
        <ULink to="#" class="hover:text-secondary transition-colors">Home</ULink>
        <ULink to="#" class="hover:text-secondary transition-colors">Why it matters</ULink>
        <ULink to="#" class="hover:text-secondary transition-colors">The venture at a glance</ULink>
      </nav>
      ...
      <button class="lg:hidden flex items-center justify-center w-10 h-10 rounded-full text-primary ml-2 shrink-0"
    ...
    <!-- Mobile Dropdown -->
    <div v-if="mobileOpen"
      class="lg:hidden mt-4 pt-4 border-t border-gray-200 flex flex-col gap-3 text-sm uppercase font-medium text-primary">
