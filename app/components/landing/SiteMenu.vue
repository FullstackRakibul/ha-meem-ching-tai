...
<template>
  <div id="site-menu " :class="[
    'site-menu fixed inset-0 z-50 mt-8 flex flex-col bg-white text-slate-900 transition-transform duration-700 ease-[cubic-bezier(0.76,0,0.24,1)]',
    menuOpen ? 'translate-y-0' : '-translate-y-full',
    { 'site-menu--has-hover': hoveredItem !== null }
  ]" aria-modal="true" role="dialog">
    <!-- Header Area -->
    <div class="site-menu__header flex justify-between items-center p-6 lg:px-12 lg:py-8 shrink-0">
      <a href="#top" class="site-menu__logo font-bold text-2xl tracking-wide outline-none group"
    ...
    <div class="site-menu__nav flex-1 w-full overflow-y-auto overflow-x-hidden flex items-center">
      <div
        class="site-menu__nav-inner w-full max-w-[1400px] mx-auto px-6 lg:px-12 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-center">

        <!-- Links Column: Taking up 7 columns on desktop to ensure long names fit perfectly -->
        <div class="site-menu__nav-links lg:col-span-7 flex flex-col gap-6 lg:gap-8 py-10">
          <a v-for="(item, index) in navItems" :key="item.label" :href="item.href"
            ...
            @mouseenter="hoveredItem = item.image" @mouseleave="hoveredItem = null" @click="emit('closeMenu')">
            <!-- Index Number -->
            <span
        ...
        <div
          ...
          <ClientOnly>
            ...
            <WavyPreview :src="hoveredItem" :is-visible="hoveredItem !== null" class="w-full h-full object-cover" />
          </ClientOnly>
        </div>
      </div>
    </div>
    ...
    <div
      ...
      <a href="#contact" class="mt-2 sm:mt-0 hover:text-slate-900 transition-colors font-medium">Contact Support</a>
    </div>
  </div>
</template>